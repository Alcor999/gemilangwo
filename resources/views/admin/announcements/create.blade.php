...
@section('title', 'Buat Pengumuman Baru - Administrator')
...
<div class="space-y-8 pb-12 max-w-3xl mx-auto">
    ...
    <div class="space-y-1">
        ...
        <h1 class="font-serif text-4xl text-choco-900 italic">Buat <span class="not-italic text-stone-300">Pengumuman</span></h1>
        <p class="text-stone-400 text-sm">Isi formulir di bawah untuk membuat pengumuman baru.</p>
    </div>
    ...
    <nav class="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-stone-400">
        ...
        <span class="text-choco-900">Buat Baru</span>
    </nav>

    <!-- Form Card -->
    <x-luxury.card class="p-8 border-stone-100 shadow-sm">
        <form action="{{ route('admin.announcements.store') }}" method="POST" class="space-y-7">
            @csrf
            ...
            <div class="space-y-2">
                ...
                <textarea id="isi"
                          ...
                          class="w-full px-5 py-4 rounded-2xl border border-stone-200 bg-stone-50/50 text-choco-900 text-sm font-light leading-relaxed
                                 placeholder:text-stone-300 focus:outline-none focus:ring-2 focus:ring-gold-300 focus:border-gold-300
                                 transition-all resize-none @error('isi') border-rose-300 bg-rose-50/50 @enderror">{{ old('isi') }}</textarea>
                @error('isi')
                    <p class="text-rose-500 text-xs flex items-center gap-1"><i class="fas fa-exclamation-circle"></i> {{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="space-y-2">
                ...
                <select id="kategori"
                    ...
                    <option value="info"       {{ old('kategori') == 'info'       ? 'selected' : '' }}>📢 Informasi</option>
                    <option value="promo"      {{ old('kategori') == 'promo'      ? 'selected' : '' }}>🎉 Promosi</option>
                    <option value="peringatan" {{ old('kategori') == 'peringatan' ? 'selected' : '' }}>⚠️ Peringatan</option>
            ...
            <div class="flex items-center gap-4 p-5 rounded-2xl bg-stone-50/80 border border-stone-100">
                ...
                <div>
                    ...
                    <p class="text-[10px] text-stone-400 uppercase tracking-widest">Pengumuman akan langsung ditampilkan</p>
                </div>
            </div>
